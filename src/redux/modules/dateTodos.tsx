...
export const getDateTodo : any = createAsyncThunk(
  ...
  async (payload:date, thunkAPI) => {
    ...
    try {
      const data = await axios.get(`http://54.180.79.105/api/v1/todoCategories/dates?selectDate=${payload}`,
      {
        headers: {
...
export const deleteCategory : any = createAsyncThunk(
  ...
  async (payload:any, thunkAPI) => {
    ...
    try {
      const data = await axios.delete(`http://54.180.79.105/api/v1/todoCategories/${payload}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJrbXNAZ21haWwuY29tIiwiaXNzIjoiaGFuZ2hhZTVfc2VhdHVkeSIsImV4cCI6MTY2MjY0Mjk5Mn0.gGbOTw4oyHuqpxoxtQjti_ITyJxZ4-tqn2fi6HOH7WI'
...
export const _editCategory : any = createAsyncThunk(
  ...
  async (payload:any, thunkAPI) => {
    ...
    try {
      const data = await axios.put(`http://54.180.79.105/api/v1/todoCategories/${payload.categoryId}`,
      {
        categoryName: payload.categoryName,
      },
      {
